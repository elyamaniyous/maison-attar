...
const MOCK_USERS: User[] = [
  { id: "user-01", name: "Administrateur", email: "admin@maison-attar.com", role: "admin", lastLogin: "2024-01-15T10:30:00" },
  { id: "user-02", name: "Éditeur Web", email: "editor@maison-attar.com", role: "editor", lastLogin: "2024-01-14T09:00:00" },
];
...
export default function UtilisateursPage() {
  ...
  const currentUser: User | undefined = users.find((u) => u.role === "admin");
  ...
  async function copyCredentials(user: User, password?: string) {
    const siteUrl = typeof window !== "undefined" ? window.location.origin : "https://maison-attar.com";
    const text = [
      "Votre accès Maison Attar Admin :",
      `URL : ${siteUrl}/admin`,
      `Email : ${user.email}`,
      password ? `Mot de passe : ${password}` : "Mot de passe : (tel que défini)",
    ].join("\n");
    try {
    ...
    <div className="max-w-3xl space-y-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="text-[#4A4A4A] text-sm mt-0.5">{users.length} compte{users.length > 1 ? "s" : ""} administrateur</p>
        </div>
        <button
          onClick={() => { setShowForm(true); setForm(emptyForm); setErrors({}); setCreated(false); }}
                  ...
                  <div className="flex items-center gap-2 justify-end">
                    ...
                    {deleteConfirm === user.id ? (
                      <div className="flex items-center gap-1.5">
                        ...
                        <button onClick={() => deleteUser(user.id)} className="text-xs text-[#B85C5C] hover:text-red-400 transition-colors">Oui</button>
      ...
      <div className="bg-[#1A1A1A] border border-[#262626] rounded-lg overflow-hidden">
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="bg-[#111111] border border-[#262626] rounded p-4 space-y-1 font-mono text-xs text-[#8A8A8A]">
            ...
            <p><span className="text-[#C4A265]">Email :</span> {currentUser?.email ?? "admin@maison-attar.com"}</p>
